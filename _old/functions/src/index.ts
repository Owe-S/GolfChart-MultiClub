...
/**
 * Secure developer-only seeding endpoint.
 * Creates default carts if missing.
 * Protected via a bearer token stored as a Firebase Functions secret.
 */
const SEED_TOKEN_SECRET = defineSecret('SEED_TOKEN');
export const seedCarts = https.onRequest({
    region: 'europe-west3',
    secrets: [SEED_TOKEN_SECRET]
}, async (req, res) => {
    corsHandler(req, res, async () => {
        ...
        // Access the secret value from the defineSecret object
        const expected = SEED_TOKEN_SECRET.value();
        ...
        try {
            const carts = [
                { id: 1, name: 'Blå 4', status: 'available', currentRentalId: null },
                { id: 2, name: 'Blå 5', status: 'available', currentRentalId: null },
                { id: 3, name: 'Grønn', status: 'available', currentRentalId: null },
                { id: 4, name: 'Hvit', status: 'available', currentRentalId: null },
                { id: 5, name: 'Svart', status: 'available', currentRentalId: null },
            ];

            const batch = db.batch();
            for (const cart of carts) {
                const ref = db.collection('carts').doc(String(cart.id));
                batch.set(ref, cart, { merge: true });
            }
            await batch.commit();

            res.status(200).send({ success: true, seeded: carts.length });
        } catch (error: any) {
            console.error('Seed error:', error);
            res.status(500).send({ error: error.message || 'Seed failed' });
        }
    });
});
 ...
 */
export const createRental = https.onRequest({ region: 'europe-west3' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
 ...
 */
export const checkAvailability = https.onRequest({ region: 'europe-west3' }, async (req, res) => {
    corsHandler(req, res, async () => {
